...
class LustreVscScratchFs(LustreVscFS):
    ...
    def pjid_from_name(self, name):
        ...
        raise ValueError("Can not use pjid_from_name")
...
class LustreOperations(PosixOperations, metaclass=Singleton):
    ...
    def _execute_lctl_get_param_qmt_yaml(self, device, typ, quotyp=Quotyp2Param.block, qmt_direct=True):
        ...
        try:
            ...
            self.log.raiseException(
                f"_execute_lctl_get_param_qmt_yaml: Error in yaml output: {exc}", LustreOperationError
            )
    ...
    def list_filesystems(self, device=None):
            ...
            if fs[0] == "lustre":
                ...
                if len(fsloc) == 2:
                    ...
                    if not devices or fsname in devices:
                        ...
                        lustrefss[fsname] = {"defaultMountPoint": fs[1], "location": fsloc[0]}
        ...
        if not devices and not lustrefss:
            ...
            self.log.raiseException(
                f"Not all Lustre Filesystems of {devices} found, found {lustrefss}", LustreOperationError
            )
    ...
    def get_project_id(self, project_path, existing=True):
        ...
        if flag == "P" and path == project_path:
            ...
            self.log.raiseException(
                f"Something went wrong fetching project id for {project_path}. Output was {res}", LustreOperationError
            )
        else:
            ...
            for pjline in res.splitlines():
                ...
                if int(pjid) == 0:
                    ...
                    if pjid in filesets:
                        ...
                        self.log.raiseException(
                            f"Project inheritance flag not set for project {pjid}: {path}", LustreOperationError
                        )
                    else:
    ...
    def make_fileset(
        self, new_fileset_path, fileset_name, inodes_max=1048576, fileset_id=None, parent_fileset_name=None
    ):
        """
        ...
        if fsinfo:
            ...
            self.log.raiseException(
                f"Found existing fileset {fileset_name} with the same name at {fsinfo['path']}", LustreOperationError
            )
            return None
        ...
        if pjid in filesets[fsname]:
            self.log.raiseException(
                f"Found existing projectid {pjid} in file system {fsname}: {filesets[pjid]}", LustreOperationError
            )
        ...
        try:
            ...
            self._set_quota(
                ...
                soft=blockq,
                hard=blockq,
                inode_soft=inodes_max,
                inode_hard=inodes_max,
            )
    ...
    def set_user_quota(self, soft, user, obj=None, hard=None, inode_soft=None, inode_hard=None):
        ...
        """
        self._set_quota(
            who=user, obj=obj, typ=Typ2Opt.user, soft=soft, hard=hard, inode_soft=inode_soft, inode_hard=inode_hard
        )
    ...
    def set_group_quota(self, soft, group, obj=None, hard=None, inode_soft=None, inode_hard=None):
        ...
        """
        self._set_quota(
            who=group, obj=obj, typ=Typ2Opt.group, soft=soft, hard=hard, inode_soft=inode_soft, inode_hard=inode_hard
        )
    ...
    def _set_quota(self, who, obj, typ=Typ2Opt.user, soft=None, hard=None, inode_soft=None, inode_hard=None):
        ...
        if soft is None and inode_soft is None:
            self.log.raiseException(
                "setQuota: At least one type of quota (block,inode) should be specified", LustreOperationError
            )
        ...
        if soft:
            if hard is None:
                ...
                self.log.raiseException(
                    f"setQuota: can't set hard limit {hard} lower then soft limit {soft}", LustreOperationError
                )
